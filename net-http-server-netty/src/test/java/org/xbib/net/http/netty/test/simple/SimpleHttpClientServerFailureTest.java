...
class SimpleHttpClientServerFailureTest {
    ...
    void testHttp() throws Exception {
        ...
        try {
            ...
            Socket socket = new Socket(InetAddress.getByName(inetSocketAddress.getHostString()), inetSocketAddress.getPort());
            PrintWriter pw = new PrintWriter(socket.getOutputStream());
            pw.println("FOOBAR /::{} HTTP/1.1");
            pw.println("Host: " + inetSocketAddress.getHostString() + ":" + inetSocketAddress.getPort());
            pw.println("");
            pw.flush();
            BufferedReader br = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            String t;
            while ((t = br.readLine()) != null) {
                clientLogger.log(Level.INFO, t);
            }
            br.close();
    ...
    private static class ClientHandler extends ChannelDuplexHandler {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            if (msg instanceof FullHttpResponse) {
                ...
                clientLogger.log(level, "msg received on client " + httpResponse + ", completing future");
        ...
        public void channelReadComplete(ChannelHandlerContext ctx) {
            clientLogger.log(level, "read complete");
            ctx.flush();
            ctx.close();
        }
    ...
    private static class ServerHandler extends ChannelDuplexHandler {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            ...
            if (msg instanceof FullHttpRequest) {
                ...
                serverLogger.log(Level.INFO, "got method: " + httpMethod);
                String hostAndPort = httpRequest.headers().get(HttpHeaderNames.HOST);
                try {
                    URI uri = URI.create("http://" + hostAndPort + httpRequest.uri());
                    serverLogger.log(Level.INFO, "got URI: " + uri);
                    if (httpMethod.equals(HttpMethod.GET)) {
                        FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
                                HttpResponseStatus.OK);
                        serverLogger.log(Level.INFO, "writing OK response: " + response + " channel writable = " + ctx.channel().isWritable());
                        ctx.write(response);
                    } else {
                        FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
                                HttpResponseStatus.BAD_REQUEST);
                        serverLogger.log(Level.INFO, "writing BAD REQUEST response: " + response + " channel writable = " + ctx.channel().isWritable());
                        ctx.write(response);
                    }
                } catch (Exception e) {
                    FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
        ...
        public void channelReadComplete(ChannelHandlerContext ctx) {
            ...
            serverLogger.log(Level.INFO, "flush and close");
            ctx.flush();
            ctx.close();
        }
